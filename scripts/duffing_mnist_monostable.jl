# Monostable Duffing on REAL MNIST (28x28), not the sklearn 8x8 digits.
#
# Our best digit result (0.96) was on scikit-learn load_digits (8x8). This tests
# whether the same monostable single-basin Duffing setup holds up on genuine MNIST
# (28x28, MLDatasets -- no PyCall). MNIST is 784-d; a full-resolution 2nd-order
# Duffing relaxation (N ~ 800) is impractical, so we 2x2-average-pool to 14x14
# (196 inputs, N ~ 246) and use a balanced subset -- a tractable but real test.
#
# Setup identical to the sklearn full-res run (results/ep_duffing_digits_mono_fullres.md):
# monostable hidden (a>0), linear/softmax readout, symmetric +-beta gradient,
# Landau/deterministic annealing, best-checkpoint, layered input->hidden->output.
# Baselines: logreg + MLP on the same 14x14 features.
#
# Run: julia -t auto --project=. scripts/duffing_mnist_monostable.jl

ENV["DATADEPS_ALWAYS_ACCEPT"] = "true"   # allow MNIST auto-download without prompt
...
# The earlier single-seed, test-selected number for this script was 0.85.

const SEEDS=1:parse(Int,get(ENV,"MN_V1_SEEDS","3")); const VAL_FRAC=0.2
const CLASSES=collect(0:9); const N_TRAIN_PC=100; const N_TEST_PC=50
const N_HID=40; const T_MAX=40.0; const DELTA=1.0; const BETA=0.1; const LR=0.008
const N_ITER=parse(Int,get(ENV,"MN_V1_ITER","300")); const BATCH=100; const ANNEAL_FRAC=0.4
const A_OP=0.5; const A_HI=3.0; const C_H=1.0
const OUTFILE=joinpath(@__DIR__,"..","results","ep_duffing_mnist_monostable_seeds.json")
const EVAL_EVERY=25; const STEADY_TOL=1e-3

println("threads = ", Threads.nthreads(), ", REAL MNIST 28x28 -> 14x14 pool, monostable Duffing")

# ---- load MNIST (MLDatasets, no PyCall), 2x2 avg-pool 28x28 -> 14x14 (196-d) ----
function pool2x2_28(img)  # img: 28x28
...
println("MNIST loaded: train ", size(tr_raw.features), ", test ", size(te_raw.features))

function subset(ds, npc, rng)
    X = Float64.(ds.features); y = ds.targets      # 28x28xN, [0,1]; labels 0-9
    ...
    return permutedims(reduce(hcat, feats)), labs   # (n,196), labels
end
...
# ---- dynamics (threaded), monostable hidden + linear output + softmax-CE nudge ----
...
println("-"^68)
@printf("%-28s | %-16s %-16s\n","model","train","test")
@printf("%-28s | %-16s %-16s\n","Duffing mono (MNIST) (val-sel)",msfmt(dutr),msfmt(du))
