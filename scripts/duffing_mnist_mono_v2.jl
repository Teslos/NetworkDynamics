...
# The earlier single-seed, test-selected number for this script was 0.904.

const SEEDS=1:parse(Int,get(ENV,"MN_V2_SEEDS","3")); const VAL_FRAC=0.2
const CLASSES=collect(0:9); const N_TRAIN_PC=200; const N_TEST_PC=100
const N_HID=60; const T_MAX=40.0; const DELTA=1.0; const BETA=0.1; const LR=0.006
const N_ITER=parse(Int,get(ENV,"MN_V2_ITER","500")); const BATCH=150; const ANNEAL_FRAC=0.35
const A_OP=0.5; const A_HI=3.0; const C_H=1.0
const OUTFILE=joinpath(@__DIR__,"..","results","ep_duffing_mnist_mono_v2_seeds.json")
...
println("-"^68)
@printf("%-28s | %-16s %-16s\n","model","train","test")
@printf("%-28s | %-16s %-16s\n","Duffing mono v2 (MNIST) (val-sel)",msfmt(dutr),msfmt(du))
