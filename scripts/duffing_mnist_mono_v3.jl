...
const SEED=1; const CLASSES=collect(0:9); const N_TRAIN_PC=500; const N_TEST_PC=100
const N_HID=60; const T_MAX=40.0; const DELTA=1.0; const BETA=0.1; const LR=0.006
const N_ITER=800; const BATCH=150; const ANNEAL_FRAC=0.30
...
println("-"^56)
@printf("%-26s | %-8.3f %-8.3f\n","Duffing mono v3 (MNIST)",du_tr,du_te)
@printf("%-26s | %-8s %-8.3f\n","logreg 14x14","-",lr_te)
@printf("%-26s | %-8s %-8.3f\n","MLP 14x14","-",ml_te)
println("\nRef: MNIST v2 = 0.904 (logreg 0.887, MLP 0.912); v1 = 0.85")
