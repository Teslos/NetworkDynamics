"""
run_classification_kuramoto.jl

Kuramoto oscillator reservoir computing classification on the optdigits dataset.
Uses a network of coupled Kuramoto phase oscillators as a dynamical reservoir,
then trains a Flux dense-layer readout for 10-class digit classification.

Kuramoto model: dθᵢ/dt = ωᵢ + Iᵢ(t) + K * Σⱼ sin(θⱼ − θᵢ)

Usage:
    julia --project=. scripts/run_classification_kuramoto.jl
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
const N_RESERVOIR   = 16      # Kuramoto oscillator nodes in the reservoir
...
println("=" ^ 60)
println("Kuramoto Reservoir Computing — Digit Classification")
...
# ── 2. Build Kuramoto reservoir network ─────────────────────────────────────
println("\n[2/5] Building Kuramoto reservoir...")

# Kuramoto parameters
const K_coupling = 0.5        # coupling strength
# Natural frequencies: spread around 1.0 to break symmetry
const ω_natural = 1.0 .+ 0.3 * randn(rng, N_RESERVOIR)

# Watts-Strogatz small-world topology
const W_adj = Float64.(adjacency_matrix(watts_strogatz(N_RESERVOIR, 4, 0.3; rng=rng)))
println("  Reservoir: $N_RESERVOIR Kuramoto oscillators, Watts-Strogatz(k=4, β=0.3)")
println("  Coupling K=$K_coupling, ω ∈ [$(round(minimum(ω_natural),digits=2)), $(round(maximum(ω_natural),digits=2))]")
...
# Kuramoto ODE: dθᵢ/dt = ωᵢ + Iᵢ(t) + K * Σⱼ Aᵢⱼ sin(θⱼ − θᵢ)
function kuramoto_reservoir!(du, u, p, t)
    N = N_RESERVOIR
    for i in 1:N
        θi = u[i]
        I_ext = CURRENT_INPUTS[i](t)

        # Sinusoidal coupling from neighbours
        coupling = 0.0
        for j in 1:N
            if W_adj[i, j] > 0
                coupling += sin(u[j] - θi)
        ...
        du[i] = ω_natural[i] + I_ext + K_coupling * coupling
    end
    nothing
end

"""
Run one sample through the Kuramoto reservoir and extract features.
Kuramoto has 1 state var per node (phase θ), so we extract sin/cos
to get meaningful Euclidean features from the circular phase.
"""
function process_sample(features::Vector{Float64})
    ...
    x0 = 2π * rand(rng, N_RESERVOIR)
    tspan = (0.0, ODE_TSPAN)
    saveat = range(0.0, ODE_TSPAN, length=N_SNAPSHOTS + 1)[2:end]

    prob = ODEProblem(kuramoto_reservoir!, x0, tspan)
    ...
    if sol.retcode == ReturnCode.Success || sol.retcode == ReturnCode.Default
        phases = hcat(sol.u...)  # (N, N_SNAPSHOTS)

        # Convert phases to sin/cos for Euclidean feature space
        sin_states = sin.(phases)   # (N, N_SNAPSHOTS)
        cos_states = cos.(phases)   # (N, N_SNAPSHOTS)

        # Snapshot features (sin + cos at each timepoint)
        snapshot_feat = vcat(vec(sin_states), vec(cos_states))  # 2*N*N_SNAPSHOTS

        # Summary statistics on sin/cos
        mean_sin = vec(mean(sin_states, dims=2))   # N
        mean_cos = vec(mean(cos_states, dims=2))   # N
        var_sin  = vec(var(sin_states, dims=2))    # N
        var_cos  = vec(var(cos_states, dims=2))    # N

        # Order parameter at each snapshot: R = |1/N Σ exp(iθⱼ)|
        order_params = [abs(mean(exp.(im .* phases[:, k]))) for k in 1:N_SNAPSHOTS]

        # Phase velocity (dθ/dt proxy) at final time
        if size(phases, 2) >= 2
            dθ = phases[:, end] .- phases[:, end-1]
        else
            dθ = zeros(N_RESERVOIR)
        end

        return Float32.(vcat(snapshot_feat, mean_sin, mean_cos, var_sin, var_cos,
                             order_params, dθ))
    else
        n_feat = 2 * N_RESERVOIR * N_SNAPSHOTS + 4 * N_RESERVOIR + N_SNAPSHOTS + N_RESERVOIR
        return zeros(Float32, n_feat)
    end
end

# Feature dims: 2*N*S snapshots + 4*N stats + S order params + N phase velocity + 64 raw pixels
reservoir_feat_dim = 2 * N_RESERVOIR * N_SNAPSHOTS + 4 * N_RESERVOIR + N_SNAPSHOTS + N_RESERVOIR
...
ax = Axis(fig[1, 1],
    title="Kuramoto Reservoir — Digit Classification (Test: $(test_acc)%)",
...
out_path = joinpath(CONF_DIR, "kuramoto_reservoir_confusion_matrix.png")
CairoMakie.save(out_path, fig)
println("  Saved: $out_path")

conf_txt_path = joinpath(CONF_DIR, "kuramoto_reservoir_confusion_matrix.txt")
open(conf_txt_path, "w") do io
    println(io, "Kuramoto Reservoir Digit Classification Results")
    println(io, "=" ^ 50)
    println(io, "Reservoir: $N_RESERVOIR Kuramoto oscillators (Watts-Strogatz small-world)")
    println(io, "Coupling K=$K_coupling, natural freq spread ω ~ N(1.0, 0.3)")
    println(io, "Features: $(feature_dim) dims (sin/cos snapshots + stats + order param + raw pixels)")
...
println("\n✓ Kuramoto classification complete.")
