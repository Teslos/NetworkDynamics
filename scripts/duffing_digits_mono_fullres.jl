...
# pooling) -- equal footing with XY Stage 3 (which used full 64px, 0.94) -- to see
# how far the monostable Duffing climbs at full resolution. Same substrate/training
# as mono v2: single-well hidden (a>0), linear/softmax readout, symmetric +-beta
# gradient, Landau annealing, best-checkpoint. Only the input resolution changes.
#
...
const SEED=1; const CLASSES=collect(0:9); const N_TRAIN_PC=100; const N_TEST_PC=70   # Wang split
...
Xtr_r=X_ALL[tr,:]./16.0; Xte_r=X_ALL[te,:]./16.0
ytr=[cc[c] for c in Y_ALL[tr]];yte=[cc[c] for c in Y_ALL[te]];Xtr=2 .*Xtr_r.-1;Xte=2 .*Xte_r.-1;Nd=length(ytr)
Ytr=[ytr[i]==j ? 1.0 : 0.0 for i in eachindex(ytr),j in 1:N_CLS]
println("N=$N (64 in, $N_HID monostable-hidden, 10 linear out), train=$Nd test=$(length(yte))\n")
...
lr_te=logreg_acc(Xtr_r,ytr,Xte_r,yte,N_CLS);ml_te=mlp_acc(Xtr_r,ytr,Xte_r,yte,N_CLS)
@printf("\ntrained %d iters in %.0fs\n",N_ITER,time()-t0)
println("="^54)
@printf("%-24s | %-8s %-8s (chance %.3f, 64px)\n","model","train","test",1/N_CLS)
println("-"^54)
@printf("%-24s | %-8.3f %-8.3f\n","Duffing mono 64px",du_tr,du_te)
@printf("%-24s | %-8s %-8.3f\n","logreg 64px","-",lr_te)
@printf("%-24s | %-8s %-8.3f\n","MLP 64px","-",ml_te)
println("\nRef: Duffing mono 4x4 = 0.84; XY full-64px = 0.94")
