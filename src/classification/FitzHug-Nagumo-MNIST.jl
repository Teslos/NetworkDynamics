...
function plot_sol(u, t_steps, num_sol)
    ...
    fig
    CairoMakie.save("FitzHug-Nagumo_MNIST_0_48.png", fig)
end
# test chaos 
# using .ChaosFHN
...
@save "models/model_flux_fhn_mnist.bson" model_flux
...
function plot_confusion_matrix_percent(mach, x_df, y_true, title_str="Confusion Matrix (%)")
    conf = confusion_matrix(mach, x_df, y_true)
...
fig_percent = plot_confusion_matrix_percent(model_flux, test_x', test_y, "Confusion Matrix (%) - σ=0.72")
CairoMakie.save("confusion_matrix_percent_sigma_0_72.png", fig_percent)
...
fig
save("confusion_matrix_sigma_0_72.png", fig)
plot_sol(Array(test_x), collect(1:size(test_x,2)), 2)
# plot the chaos analysis of the FitzHugh-Nagumo network
using .ChaosFHN
# plot the phase space of the first 5 oscillators
println("\n1. CHAOS ANALYSIS")
tspan = (0.0, Float64(size(test_x,2)))
datasize = size(test_x,2)
tsteps = range(tspan[1], tspan[2], length=datasize)
fig_phase = ChaosFHN.analyze_phase_space(test_x, 1:5, colormap=:viridis, use_gpu=true, tsteps=tsteps)
