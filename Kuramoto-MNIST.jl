...
@inline Base.@propagate_inbounds function kivertex_simple!(dv, v, edges, p, t)
    ψ = p
    #println("h: ", h, " ψ: ", ψ)
    if t < forcing_period
        dv .= -β * h*sin.(v .- ψ(t))
...
@inline Base.@propagate_inbounds function kiedge_simple!(e, v_s, v_d, p, t)
    w = p
    if t < forcing_period
...
odeelevertex = ODEVertex(; f=kivertex_simple!, dim=1, sym=[:u])
odeeleedge = StaticEdge(; f=kiedge_simple!, dim=1, coupling=:directed)
...
const β = 10.0 # parameter for the Kuramoto model
const forcing_period = 2000.0 # forcing period for the Kuramoto model
const h = 0.5 # coupling strength for the Kuramoto model
...
function plot_sol(u, t_steps, num_sol)
    ...
    fig
    GLMakie.save("Kuramoto_MNIST.png", fig)
end
using Plots
